...
class DataQualityChecker:
    ...
    @staticmethod
    def check_matching_records(customers_df_sqlite,customers_df_tinydb):
            sample_records_sqlite = customers_df_sqlite.sample(n=15)
            sample_records_tinydb = customers_df_tinydb.sample(n=15)
            matching_records = sample_records_sqlite.merge(sample_records_tinydb, left_on='email', right_on='email')
            if matching_records.empty:
                return "Migration testing passed. Sample records from SQLite and TinyDB match."
            else:
                return "Migration testing failed. Inconsistent sample records found between SQLite and TinyDB \n"+matching_records
